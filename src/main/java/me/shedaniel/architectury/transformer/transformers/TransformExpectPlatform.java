...
import me.shedaniel.architectury.transformer.ClassTransformer;
...
public class TransformExpectPlatform implements Transformer {
    @Override
    public void transform(Path input, Path output) throws Throwable {
        Transform.transform(input, output, transformExpectPlatform());
    }
    
    public ClassTransformer transformExpectPlatform() {
        boolean[] injectedClass = {!RemapInjectables.isInjectInjectables()};
        
        return (clazz, classAdder) -> {
            if (!injectedClass[0]) {
                injectedClass[0] = true;
                try (InputStream stream = TransformExpectPlatform.class.getResourceAsStream("/annotations-inject/injection.jar")) {
                    ZipUtil.iterate(stream, (input, entry) -> {
                        if (entry.getName().endsWith(".class")) {
                            ...
                            try {
                                classAdder.add(newName, writer.toByteArray());
                            } catch (Exception e) {
